...
export class HotelInnerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe((response) => {
      ...
      this.hotelCardServ
        .readHotelCardById(this.hotelId)
  ...
  bookRoom(roomName: string) {
    this.hotel.rooms.forEach((room: any) => {
      if (room.name == roomName) {
        ...
        let pricePerNight = document.getElementById(
          "pricePerNight"
        ) as HTMLInputElement;
        let price = document.getElementById("price") as HTMLInputElement;
        let roomName = document.getElementById("roomName") as HTMLInputElement;
        let daysPriceSum = document.getElementById(
          "daysPriceSum"
        ) as HTMLInputElement;
        ...
        if (this.valid == 1) {
          ...
          this.dayCountSumPrice =
            this.chosenRoom.price * parseInt(this.dayCount);
  ...
  Calculate() {
    if (
      ...
      this.dayCount = Math.round(
        (this.dateRange.value.end - this.dateRange.value.start) / oneDay
      ).toString();
  ...
  continueToBookTheRoom() {
    this.bookedHotelData.push({
      userId: this.userData.uid,
      hotelId: this.hotelId,
      hotelName: this.hotel.name,
      roomName: this.chosenRoom.name,
      pricePerNight: this.chosenRoom.price,
      priceSum: this.dayCountSumPrice,
      dayCount: this.dayCount,
      checkInDate: this.dateRange.value.start,
      checkOutDate: this.dateRange.value.end,
      guestsCount: this.chosenRoom.personsCount,
      imageUrl: this.hotel.mainImages[0],
      expiry: this.currentDate,
    });
    localStorage.setItem(
      "bookedHotelData",
      JSON.stringify(this.bookedHotelData)
    );
  }
}
