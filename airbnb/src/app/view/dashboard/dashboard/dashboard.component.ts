...
export class DashboardComponent implements OnInit {
  ...
  constructor(
    private hotelBookingServ: HotelBookingService,
  ...
  ngOnInit() {
    ...
    this.backToBookingHotel = JSON.parse(
    ...
    if (this.backToBookingHotel != null) {
      if (this.backToBookingHotel[0].userId == this.userData.uid) {
        this.backToBookingHotelExpiryDate = new Date(
          this.backToBookingHotel[0].expiry
        );
        this.expirationDayCount =
          (this.currentDate.getTime() -
            this.backToBookingHotelExpiryDate.getTime()) /
          oneDay;
